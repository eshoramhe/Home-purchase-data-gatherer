...
def extract_real_estate_data(soup):
    data = {
        "Address": None,
        "Cost to Buy": None,
        "Cost to Rent": None,
        ...
        "Extracted URL": None  # To store the URL for which data was extracted
    }
    ...
    price_elements = soup.find_all(
        text=re.compile(r'\$\s*\d{1,3}(?:,\d{3})*(?:\.\d{2})?|R\s*\d{1,3}(?:,\d{3})*(?:\.\d{2})?',
                        re.I))  # Matches $ or R followed by numbers
    for element in price_elements:
        ...
        if not data["Cost to Buy"] and not data["Cost to Rent"] and len(
                element.strip()) > 3:  # Avoid very short numbers
    ...
    house_match = re.search(
        r'(?:house|home|living|interior|building)\s*(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s*(sq\.?\s*ft\.?|m\u00B2|m2|square\s*feet|sqft)',
        all_text, re.I)
    plot_match = re.search(
        r'(?:lot|plot|land|property|garden)\s*size\s*(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s*(sq\.?\s*ft\.?|m\u00B2|m2|square\s*feet|sqft)',
        all_text, re.I)
    ...
    if not url_input:
        ...
        with st.spinner("Fetching and extracting data..."):
            try:
                ...
                # Extract data
                extracted_data = extract_real_estate_data(soup)

                if any(value is not None for key, value in extracted_data.items() if
                       key != "Extracted URL"):  # Check if any data was extracted
                ...
                st.error(
                    "Error: Could not connect to the website. Please check your internet connection or if the URL is correct.")
            except requests.exceptions.Timeout:
                st.error("Error: The request timed out. The server might be slow or the URL is unreachable.")
            except requests.exceptions.HTTPError as e:
                st.error(
                    f"Error fetching the URL (HTTP {e.response.status_code}): {e}. The server denied access or the page does not exist.")
